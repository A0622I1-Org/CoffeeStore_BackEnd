...
public class UserEditByRequestDtoValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        } else if (!Pattern.compile("^[a-zA-Z\'-\'\\sáàảãạăâắằấầặẵẫậéèẻ ẽẹếềểễệóêòỏõọôốồổỗộ ơớờởỡợíìýỉĩịđùỳúủũụưứ� �ửữựÀÁÂÃÈÉÝÊÌÍÒÓÔÕÙÚĂĐĨŨƠ ƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼ� ��ỀỂỄỆỈỊỌỎỐỒỔỖỘỚỜỞ ỠỢỤỨỪỬỮỰỲỴÝỶỸửữựỵ ỷỹ]*$").matcher(userEditDTO.getName()).find()) {
        ...
        } else if (!Pattern.compile("^[a-zA-Z0-9\'-\'\\sáàảãạăâắằấầặẵẫậéèẻ ẽẹếềểễệóêòỏõọôốồổỗộ ơớờởỡợíìỉĩịýđùúủũụưứ� �ửữựÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠ ƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼ� ��ỀỂỄỆỈỊỌỎỐỒỔỖỘỚỜỞ ỠỢỤỨỪỬỮỰỲỴÝỶỸửữựỵ ỷỹ ,]*$").matcher(userEditDTO.getAddress()).find()) {
        ...
        } else if(!userEditDTO.getCurrentPhoneNumber().equals(userEditDTO.getCurrentPhoneNumber())) {
